...
export default function UpscalePage() {
  ...
  const doUpscale = async () => {
    ...
    setLoading(true);
    setProgress(0);
    setStatus("تحميل نموذج الذكاء الاصطناعي...");

    try {
      // dynamic imports — loaded only when user clicks
      const [{ default: Upscaler }, model] = await Promise.all([
        import("upscaler"),
        scale === 2 ? import("@upscalerjs/esrgan-slim/2x") :
        scale === 3 ? import("@upscalerjs/esrgan-slim/3x") :
        scale === 4 ? import("@upscalerjs/esrgan-slim/4x") :
                      import("@upscalerjs/esrgan-slim/8x"),
      ]);

      setStatus("تحضير المعالج...");
      ...
      setStatus("جارٍ رفع الدقة...");

      const result: string = await upscaler.upscale(img, {
        patchSize: 64,
        padding: 2,
        progress: (p: number) => {
    ...
    <main className="min-h-screen pt-28 pb-24 px-4 md:px-8 max-w-5xl mx-auto">
      ...
      <div className="mb-10 text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-black leading-tight mb-3">
          رفع الدقة <span className="text-gradient">بالذكاء الاصطناعي</span>
        </h1>
        <p className="text-gray-500 text-base max-w-lg mx-auto">
          نموذج ESRGAN يعمل مباشرة في متصفحك — صورتك لا تُرسل لأي خادم
        </p>
      </div>
      ...
      {!original ? (
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 text-sm">JPG · PNG · WEBP — حتى 10MB</p>
          </div>
          <input
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="glass-card rounded-2xl p-4 flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-400 font-bold ml-1">درجة الرفع:</span>
          ...
          <p className="text-center text-xs text-gray-700 mt-2">
            يعمل كلياً في متصفحك — لا تُرسل صورتك لأي خادم · نموذج ESRGAN Slim
          </p>
        </div>
      )}
    </main>
